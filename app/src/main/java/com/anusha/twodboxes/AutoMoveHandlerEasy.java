...
/**
 * Created by skpraman on 6/3/2017.
 */

public class AutoMoveHandlerEasy {
    private int LOG_LEVEL = 1;

    private void log(String msg, int level) {
        if (level <= LOG_LEVEL) {
            Log.d("AutoMoveHandlerEasy : " + LOG_LEVEL, msg);
        }
    }

    public void getAutoMove() {
        log("AutoMoveHandlerEasy",1);
        ...
        resetTempBoxes();
        log("AutoMoveHandlerEasy, Searching for Ready Boxes",1);
        ...
        log("AutoMoveHandlerEasy, No Ready Boxes Found. Will have to it the hard way",1);
        int minBoxes = 1000;
        ...
        log("AutoMoveHandlerEasy, Found Some Lines. Min Boxes = " + minBoxes,1);
        ...
        for (outerVar = 0; outerVar < GameHandler.numXDots; outerVar++) {
            for (innerVar = 0; innerVar < GameHandler.numYDots; innerVar++) {
                log("AutoMoveHandlerEasy, lineBoxHorz[outerVar][innerVar].getBoxNum(), lineBoxVert[outerVar][innerVar].getBoxNum() = "
                        + lineBoxHorz[outerVar][innerVar].getBoxNum() + ","
                        + lineBoxVert[outerVar][innerVar].getBoxNum(),1);
        ...
        log("AutoMoveHandlerEasy, Number of Possible Lines = " + lineCount,0);
        if (lineCount > 0) {
    ...
    private int findBoxes(int x, int y, int x1, int y1) {
        int numBoxesFound = 0;
